public class WeatherService
{
    ...
    public async Task<(double, double)?> GetCoordinates(string city)
    {
        ...
        var lat = response.Value.GetProperty("results")[0].GetProperty("latitude").GetDouble();
        var lon = response.Value.GetProperty("results")[0].GetProperty("longitude").GetDouble();
    ...
    public async Task<WeatherDetails?> GetWeather(string city)
    {
        ...
        var temp = response.Value.GetProperty("current").GetProperty("temperature_2m").GetSingle();
        return new WeatherDetails(city, temp);
    }
}